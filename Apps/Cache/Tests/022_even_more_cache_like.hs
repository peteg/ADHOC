...
-- do whatever it likes.  Also the fair choices are independent, so we
-- don't necessarily get ag (af read) and ag (af write).
process i (req, ack) =
  ...
  where
    ...
    read = catchE $ \exn ->
           (loopE_ $ \exn' -> emitE req >>> presentE ack (throwE exn') (throwE exn `nondetE` nothingE) >>> pauseE)
       >>> (pauseE -- let req be absent for an instant.
         `nondetE`
...
system = arbitratedBus procs
  where
    procs () = ( (mkSizedListf $ \i -> (procAID i, process i)) `withLength` (undefined :: Processes)
               , nothingE )
...
-- This fails as some req's are pulled before an ack is possible.
test_deadlock_freedom = isFailure (mc ctlM (ag (conjoin
...
test_no_strict_sequencing = isOK (mc ctlM (ag (conjoin
  [ ef (ai /\ (ai `eu` (conjoin [ neg (probe (ackP j)) | j <- processes ] `eu` ai)))
